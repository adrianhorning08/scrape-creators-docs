...
export default function EndpointMethod({ method, path, onTryClick }) {
  const [copied, setCopied] = React.useState(false);
  const [showTryEndpoint, setShowTryEndpoint] = React.useState(false);

  const handleCopy = async () => {
    await navigator.clipboard.writeText(`https://api.scrapecreators.com${path}`);
    setCopied(true);
    ...
    <div className="flex w-full flex-col bg-background-light dark:bg-background-dark border-standard rounded-2xl p-1.5 overflow-hidden">
      <div className="flex items-center space-x-1.5">
        <div className="relative flex-1 flex gap-2 min-w-0 rounded-xl items-center cursor-pointer p-1.5 border-standard whitespace-nowrap bg-background-light dark:bg-background-dark">
          <div className={`rounded-lg font-bold px-1.5 py-0.5 text-sm leading-5 ${methodColors[method]}`}>
            {method}
          </div>
          <div className="flex items-center space-x-2 overflow-x-auto flex-1 no-scrollbar relative">
            <div className="group flex items-center flex-1 gap-0.5 font-mono cursor-pointer" onClick={handleCopy}>
              {copied ? (
